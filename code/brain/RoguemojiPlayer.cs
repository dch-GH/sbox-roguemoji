...
public partial class RoguemojiPlayer : ThingBrain
{
	...
	public override void HearSound(string name, IntVector soundPos, Thing sourceThing, int loudness = 0, float volume = 1, float pitch = 1, bool noFalloff = false)
	{
		...
		var sound = SoundExtensions.Play(To.Single(Client), name, xOffset, yOffset);
		sound.Pitch = pitch;
		sound.Volume = volume * falloff;
	}
	...
	public void SelectThing(Thing thing, bool playSfx = false)
	{
		...
		if (SelectedThing != null)
			SelectedThing.RefreshGridPanelClient(To.All);
	...
	//[TargetedRPC]
	public void AimDirectionClient(To to)
	{
		AimingCells.Clear();
	...
	public void PlaySfxUI(string name, float volume = 1f, float pitch = 1f)
	{
		...
		var sound = SoundExtensions.Play(To.Single(Client), name, Vector3.Zero);
		sound.Pitch = (pitch);
		sound.Volume = (volume);
	}
